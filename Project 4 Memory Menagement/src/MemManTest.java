...
/**
 * This class tests the main method as well
 * as the complete funcitonality of Command
 * Parser. This class has two big input files
 * that test almost the entire of the code.
 * 
 * @author Rowan Cusano
 * @author Evan Lee
 * @version 2022.12.5
...
public class MemManTest extends TestCase {
    ...
    /**
     * Test with the example input given to us
     * 
     * @throws IOException
     */
    public void testExampleFile1() throws IOException {
        ...
        assertEquals(
            "|When Summer's Through| does not exist in the song database.\n"
                ...
                + "|(The Best Part Of) Breakin' Up| does "
                + "not exist in the song database.\n"
                + "(44,11) -> (97,28) -> (239,17)\n"
                + "|Blind Lemon Jefferson| duplicates a "
     ...
     * @throws IOException
     */
    public void testExampleFile2() throws IOException {
        systemOut().clearHistory();

        String[] args = new String[3];
        args[0] = "5";
        args[1] = "32";
        args[2] = "testInput2.txt";
        ...
        assertEquals(
            "|When Summer's Through| does not exist in the song database.\n"
                ...
                + "|Under Pressure| is added to the song database.\n"
                + "|Queen| is added to the artist database.\n"
                + "|Jingle Bells| duplicates a record"
                + " already in the song database.\n"
                + "|Queen| duplicates a record already"
                + " in the artist database.\n"
                + "|Little Eva| duplicates a record already"
                + " in the artist database.\n"
                + "|The Loco-Motion| duplicates a record"
                + " already in the song database.\n"
                + "(51,4) -> (121,4) -> (335,17)\n", out);
    }
}